...
- **현재 형태**: ECS on EC2, 인스턴스는 두 부류(arm64, `bridge` 네트워크·동적 호스트 포트). compose 의 서비스 9개가 ECS 서비스 9개로 1:1 대응한다.
  ...
  - **app** ASG(`gachamind-app`, t4g.medium, 0~2대): web/matchmaking/user/worker/game-session 샤드. `attribute:role == app` 제약 + 인스턴스 간 spread. 인스턴스는 소모품이라 desired 를 올리고 내리는 게 곧 켜고 끄기다(Terraform 은 desired 를 무시한다).
  - **켜고 끄기는 `infra/env.sh up [app 대수] | down | status`**. `down` 은 app 컨테이너 인스턴스를 DRAINING 으로 돌려 태스크를 정리한 뒤 desired 0 → core 정지. 정지 중 남는 비용은 ALB 공인 IPv4 2개·core EBS·Route53 존. 서비스 간 호출은 **ECS Service Connect** 로 compose 와 같은 이름(`http://matchmaking:4000`, `redis://redis:6379`, `postgres:5432`)을 쓴다 — 태스크마다 붙는 프록시가 이름을 실제 (인스턴스, 포트)로 풀어주므로 인스턴스가 늘어도 env 값이 안 바뀐다. Redis/Postgres/RabbitMQ 도 아직 컨테이너(Postgres 는 `/data/postgres` 호스트 볼륨).
...
- 다음 단계(미정): 관리형 데이터 계층(RDS/ElastiCache/Amazon MQ)으로 core 제거, web 태스크 오토스케일링(타깃당 요청 수) + 부하 실험, app ASG 를 capacity provider 로 묶어 자동 drain.
